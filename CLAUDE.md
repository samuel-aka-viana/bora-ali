...
- **docker-compose.yml** (root): full stack — frontend, backend, postgres, redis, VersityGW, Jaeger
...
# load test (requer create_load_test_users 100)
...
Services: `localhost` (frontend), `localhost/api/` (API), `localhost/api/health/` (health check), `:8081` (VersityGW), `:16686` (Jaeger).
...
**Password hashing**: Argon2 first, PBKDF2 as fallback for existing passwords. Requires `argon2-cffi`. Old hashes are transparently re-hashed on next login.

**DB connections**: `CONN_MAX_AGE=60` — threads reuse PostgreSQL connections for 60s instead of opening/closing per request. Use Supabase PgBouncer port 6543 in production (not 5432).
...
**Health check**: `GET /api/health/` → `{"status": "ok"}`. No auth, no DB query. Used by Docker `HEALTHCHECK` and Cloudflare uptime monitoring.

**DEBUG default**: `DJANGO_DEBUG` env var defaults to `"False"`. Always set `DJANGO_DEBUG=True` explicitly in local `.env`. Never rely on the default being True.
...
backend/
  ...
  accounts/           # auth: UserSession, UserProfile, SingleSessionJWTAuthentication
    ...
    authentication.py # SingleSessionJWTAuthentication + Redis session cache
  ...
  places/             # Place, Visit, VisitItem
    ...
    managers.py       # PlaceQuerySet, VisitQuerySet, VisitItemQuerySet — always select_related
  ...
  entrypoint.sh       # runs migrate + compilemessages, then exec gunicorn
```

Endpoints: `/api/health/` · `/api/auth/{register,login,refresh,logout,me}/` · `/api/places/` · `/api/places/{public_id}/visits/` · `/api/visits/{public_id}/items/` · `/api/media/<path>` · `/api/docs/`

## Gunicorn

`3 workers, 2 threads, worker-class gthread`. Configured in both `Dockerfile CMD` and `docker-compose.yml` (must stay in sync). `entrypoint.sh` runs migrations before starting gunicorn via `exec "$@"` (PID 1 = gunicorn for proper signal handling).
...
- gzip ativo para JS/CSS/JSON
- `/assets/` → `Cache-Control: public, max-age=31536000, immutable` (Vite gera nomes com hash)
- `/index.html` → `Cache-Control: no-cache, no-store` (SPA entry sempre fresco)
- `/api/` → proxy para `backend:8000`
- Security headers em todos os locations (add_header não herda entre locations)
...
npx playwright test auth-negative            # by name pattern
npx playwright test --grep "nega"            # regex filter
```

- Config: `frontend/playwright.config.ts` — baseURL `http://localhost:8181`
- Specs: `frontend/e2e/` — `login/`, `crud/`, `auth/`, `responsive.spec.ts`
- Support: `e2e/support/` — `auth.ts` (live backend), `mock-auth.ts` + `error-fixtures.ts` (mock-only)
- Happy-path tests need a live backend. Negative + responsive tests are fully mocked via `page.route`.

## Google OAuth

`POST /api/auth/google/` accepts `{ id_token }`. Backend verifies with Google, creates/finds user, sets `is_google_account=True`. Password change is blocked for Google accounts. Requires `GOOGLE_OAUTH_CLIENT_ID` (backend) and `VITE_GOOGLE_OAUTH_CLIENT_ID` (frontend) env vars.

## Production Checklist (quando for subir)

- [ ] `DJANGO_SECRET_KEY` forte (50+ chars) no `.env` do VPS
- [ ] `DJANGO_DEBUG=False` (já é o default)
- [ ] `POSTGRES_PORT=6543` (PgBouncer do Supabase, não porta direta)
- [ ] `PUBLIC_BASE_URL` configurado com o domínio real
- [ ] Cloudflare na frente: proxy ligado, SSL Full Strict, `/api/*` bypass cache
- [ ] `REDIS_URL` apontando para Redis local no VPS

## Out of Scope

microservices, queues, websockets, social feeds, Instagram import, payments, PWA, Facebook/Apple OAuth.
