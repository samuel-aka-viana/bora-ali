...
class GoogleLoginView(APIView):
    ...
    def post(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        logger.info("Google login request received from ip=%s", request.META.get("REMOTE_ADDR"))
        claims = google_login.verify_google_id_token(serializer.validated_data["id_token"])
        user = google_login.resolve_google_user(claims)
        logger.info("Google login succeeded for user_id=%s username=%s", user.id, user.username)
